...
int listlen(int type, WORDCELL val);                   /* list.c */
int strglen(int type,WORDCELL val);                    /* string.c */

void numeric_err(float val,EXPRPTR e,char const *s)
{ 
  char err[120];
...
void f_ne(rEXPRPTR e)
{
  ...
  switch(VStype){
    case EOD:    
      return;
    case ERROR:  
      error("1st arg of ne is  ",e,VStype,VSvalue);
      return;
    default:     
      break;
  }
